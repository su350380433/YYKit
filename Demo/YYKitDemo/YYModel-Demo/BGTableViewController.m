...
- (void) simpleModelJsonModelConvert {
    ...
    User *user = [User modelWithDictionary:json];
    NSLog(@"%@",user);
    
    // Convert model to json:
    NSDictionary *jsonConvert = [user modelToJSONObject];
    NSLog(@"%@",jsonConvert);
}
...
- (void) DoubleModelJsonModelConvert {
    ...
    // Convert json to model:
    Book *book = [Book modelWithDictionary:json];
    NSLog(@"book ===== %@",book);
    
    // Convert model to json:
    NSDictionary *jsonDict = [book modelToJSONObject];
...
- (void) DifferentJSONKeyModelConvert {
    ...
    // Convert json to model:
    DifferentJSONKey *differentJsonKey = [DifferentJSONKey modelWithDictionary:json];
    
    // Convert model to json:
    NSDictionary *jsonDict = [differentJsonKey modelToJSONObject];
    NSLog(@"jsonDict ===== %@",jsonDict);
}
...
- (void) containerJsonModelConvert {
    ...
    ContainerModel *containModel = [ContainerModel modelWithDictionary:json];
    ...
    [self.listArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        List *listModel = [List modelWithDictionary:listDict];
        //随便获取count 和 id两个类型
...
- (void) BlacklistAndWhitelistModelConvert {
    ...
    // Convert json to model:
    BlacklistAndWhitelist *blacklistAndWhitelist = [BlacklistAndWhitelist modelWithDictionary:json];
    
    // Convert model to json:
    NSDictionary *jsonDict = [blacklistAndWhitelist modelToJSONObject];
    NSLog(@"jsonDict ===== %@",jsonDict);
}

- (void) timestamp {
    NSDictionary *timeDict = [self getJsonWithJsonName:@"timestamp"];
    TimeStampModel *timestamp = [TimeStampModel modelWithDictionary:timeDict];
